...
END {
    if (count_pkg > 0) {
        ...
        if (alert) {
            print ""
            print "========================================="
            print "⚠️  WARNING: TEMPERATURE ALERT!"
            print "========================================="
            print "Server temperature exceeded 20°C"
            print "Please check the cooling system!"
            print "========================================="
        }
...
# Check if temperature alert was triggered
if echo "$OUTPUT" | grep -q "WARNING: TEMPERATURE ALERT"; then
    echo ""
    echo "🔔 Temperature alert detected! Sending email..."
    php "$HOME/send_email.php" "$DATE" "$OUTPUT"
